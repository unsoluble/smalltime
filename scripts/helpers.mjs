...
export class Helpers {
    ...
    if (
      ...
      if (SimpleCalendar.api.getAllSeasons().length == 0) {
        game.settings.set(
          "smalltime",
          "sunrise-start",
          ST_Config.SunriseStartDefault
        );
        game.settings.set(
          "smalltime",
          "sunrise-end",
          ST_Config.SunriseEndDefault
        );
        game.settings.set(
          "smalltime",
          "sunset-start",
          ST_Config.SunsetStartDefault
        );
        game.settings.set(
          "smalltime",
          "sunset-end",
          ST_Config.SunsetEndDefault
        );
      } else {
  ...
  static updateGradientStops() {
    ...
    const initialPositions = {
      ...
      sunsetEnd: Helpers.convertTimeIntegerToPosition(
        game.settings.get("smalltime", "sunset-end")
      ),
    };

    const sunriseMiddle1 = Math.round(
      (initialPositions.sunriseStart * 2) / 3 +
        (initialPositions.sunriseEnd * 1) / 3
    );
    const sunriseMiddle2 = Math.round(
      (initialPositions.sunriseStart * 1) / 3 +
        (initialPositions.sunriseEnd * 2) / 3
    );
    const sunsetMiddle1 = Math.round(
      (initialPositions.sunsetStart * 2) / 3 +
        (initialPositions.sunsetEnd * 1) / 3
    );
    const sunsetMiddle2 = Math.round(
      (initialPositions.sunsetStart * 1) / 3 +
        (initialPositions.sunsetEnd * 2) / 3
    );

    // Set the initial gradient transition points.
    document.documentElement.style.setProperty(
      "--SMLTME-sunrise-start",
      Helpers.convertTimeIntegerToPercentage(
        game.settings.get("smalltime", "sunrise-start")
      )
    );
    document.documentElement.style.setProperty(
      ...
      Helpers.convertTimeIntegerToPercentage(
        Helpers.convertPositionToTimeInteger(sunriseMiddle1)
      )
    );
    document.documentElement.style.setProperty(
      "--SMLTME-sunrise-middle-2",
      Helpers.convertTimeIntegerToPercentage(
        Helpers.convertPositionToTimeInteger(sunriseMiddle2)
      )
    );
    document.documentElement.style.setProperty(
      "--SMLTME-sunrise-end",
    ...
    document.documentElement.style.setProperty(
      ...
      Helpers.convertTimeIntegerToPercentage(
        Helpers.convertPositionToTimeInteger(sunsetMiddle1)
      )
    );
    document.documentElement.style.setProperty(
      "--SMLTME-sunset-middle-2",
      Helpers.convertTimeIntegerToPercentage(
        Helpers.convertPositionToTimeInteger(sunsetMiddle2)
      )
    );
    document.documentElement.style.setProperty(
      "--SMLTME-sunset-end",
  ...
  static setupDragHandles() {
    ...
    const initialTimes = {
      sunriseStart: Helpers.convertPositionToDisplayTime(
        initialPositions.sunriseStart
      ),
      sunriseEnd: Helpers.convertPositionToDisplayTime(
        initialPositions.sunriseEnd
      ),
      sunsetStart: Helpers.convertPositionToDisplayTime(
        initialPositions.sunsetStart
      ),
      sunsetEnd: Helpers.convertPositionToDisplayTime(
    ...
    if (sunSync) {
      Object.keys(initialTimes).forEach(
        (key) => (initialTimes[key] += syncString)
      );
    }
    ...
    sunriseStartDrag.on("dragMove", function () {
      ...
      if (this.position.x >= sunriseEndDrag.position.x - offsetBetween) {
        ...
        $(".sunrise-end").attr(
          "aria-label",
          Helpers.convertPositionToDisplayTime(shovedPos)
        );
  ...
  static setCalendarFallback() {
    ...
    if (
      ...
      game.settings.set(
        "smalltime",
        "calendar-provider",
        Helpers.getCalendarProviders()[0]
      );
    }
  }
  ...
  static handleTimeChange(timeInteger) {
    ...
    $("#hourString").html(
      SmallTimeApp.convertTimeIntegerToDisplay(timeInteger).hours
    );
    $("#minuteString").html(
      SmallTimeApp.convertTimeIntegerToDisplay(timeInteger).minutes
    );
    ...
    if (
      ...
      if (currentWorldTime < 0) {
        seconds =
          60 - Math.abs(Math.trunc(((currentWorldTime % 86400) % 3600) % 60));
      } else {
        seconds = Math.abs(
          Math.trunc(((currentWorldTime % 86400) % 3600) % 60)
        );
      }
  ...
  static getDate(provider, variant) {
    ...
    if (
      ...
      let SCobject = SimpleCalendar.api.timestampToDate(
        game.time.worldTime
      ).display;
      day = SimpleCalendar.api.timestampToDate(game.time.worldTime)
        .showWeekdayHeadings
        ? SCobject.weekday
        : undefined;
    ...
    // 12 August, 2021
    displayDate.push(
      Helpers.stringAfter(date) + Helpers.stringAfter(monthName, ", ") + year
    );

    // 12 August
    displayDate.push(
      Helpers.stringAfter(date) + Helpers.stringAfter(monthName)
    );

    // 12 / 8 / 2021
    displayDate.push(
      Helpers.stringAfter(date, " / ") +
        Helpers.stringAfter(month, " / ") +
        year
    );
  ...
  static applySaturationToHexColor(hex, saturationPercent) {
    ...
    let rgbIntensityFloatSorted = rgbIntensityFloat
        .slice(0)
        .sort(function (a, b) {
          return a - b;
        }),
  ...
  static async adjustMoonlight(phases) {
    ...
    if (!game.scenes.viewed.getFlag("smalltime", "moonlight") || !phases.length)
      return;
    let newThreshold = 0;
